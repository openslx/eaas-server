...
public class SocketResourceAdapter implements ResourceAdapter {
	...
	private final ConnectorLogger log = new ConnectorLogger(this.getClass().getName());
	...
	private <T> void sendEndpoint(MessageEndpointFactory factory, Method target, T message) {
		try {
			...
			try {
				...
				log.error("Exception on message endpoint invocation",  e.getMessage());
			}
			endpoint.afterDelivery();
			endpoint.release();
		} catch (UnavailableException e) {
			log.error("Message endpoint is unavailable", e.getMessage());
		} catch (ResourceException | NoSuchMethodException e) {
			log.error("Exception on message endpoint processing",  e.getMessage());
		}
	}
