...
public class HarvesterBackend
{
	...
	{
		this(config, http, HarvesterBackend.getDefaultFromTimestamp());
	}

	public HarvesterBackend(BackendConfig config, Client http, Instant latestItemTimestamp)
	{
		this(config, http, new HarvesterStatus(latestItemTimestamp));
	}

	public HarvesterBackend(BackendConfig config, Client http, HarvesterStatus status)
	{
		this.config = config;
		this.status = status;
		...
		this.service = new ServiceProvider(HarvesterBackend.newContext(srcurl));
	...
	public HarvesterBackend(HarvesterDescription description, Client http)
	{
		this(description.getBackendConfig(), http, description.getStatus());
	}

	public HarvestingResult execute() throws HarvestException
	{
		return this.execute(HarvesterBackend.getDefaultFromTimestamp());
	}

	public HarvestingResult execute(Instant fromts) throws HarvestException
	{
		return this.execute(fromts, HarvesterBackend.getDefaultUntilTimestamp());
	}
	...
	public synchronized HarvestingResult execute(Instant fromts, Instant untilts) throws HarvestException
	{
		...
		final HarvestingResult result = new HarvestingResult(startTimestamp, fromts, untilts);
		...
		final Stream<ItemDescription> items = HarvesterBackend.toItemStream(records)
				.peek((item) -> {
					result.onRecordDownloaded();

					// Update latest item's timestamp
					final Instant curts = Instant.parse(item.getIdentifier().getTimestamp());
					if (curts.isAfter(status.getLatestItemTimestamp()))
						status.setLatestItemTimestamp(curts);
				});
		...
		status.setLastRunTimestamp(Instant.now());
