...
/** A JSON-stream of {@link ItemIdentifierDescription}s, prefixed by a header. */
public class ItemIdentifierDescriptionStream extends JsonReader
{
	public static ItemIdentifierDescriptionStream create(Response response)
	{
		final InputStream istream = response.readEntity(InputStream.class);
		return new ItemIdentifierDescriptionStream(istream, response::close);
	}

	public static ItemIdentifierDescriptionStream create(InputStream istream)
	{
		return new ItemIdentifierDescriptionStream(istream);
	}
	...
	public Stream<ItemIdentifierDescription> getItemIdentifierStream()
	{
		JsonUtils.expect(json, JsonParser.Event.START_ARRAY);
		return json.getArrayStream()
				.map((value) -> ItemIdentifierDescription.from(value.asJsonObject()));
	}
	...
	/** Specialized writer for streaming {@link ItemIdentifierDescription}s. */
	public static class Writer extends JsonWriter
	{
		...
		public Writer write(Stream<ItemIdentifierDescription> ids) throws JsonException
		{
			super.write(Parts.IDENTIFIERS, ids, 10);
			return this;
		}
	}
	...
	private ItemIdentifierDescriptionStream(InputStream istream)
	{
		super(istream);
	}

	private ItemIdentifierDescriptionStream(InputStream istream, Runnable cleanup)
	{
